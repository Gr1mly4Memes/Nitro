    ...
    public static void updateHeightmaps(Heightmap heightmap0, Heightmap heightmap1, Heightmap heightmap2, Heightmap heightmap3, LevelChunk worldChunk, final int x, final int y, final int z, BlockState state) {
        ...
        for (int searchY = y - 1; searchY >= bottomY && heightmapsToUpdate > 0; --searchY) {
            ...
            if (heightmap0 != null && blockPredicate0.test(blockState)) {
                heightmap0.update(x, searchY, z, blockState);
                heightmap0 = null;
                heightmapsToUpdate--;
            }
            if (heightmap1 != null && blockPredicate1.test(blockState)) {
                heightmap1.update(x, searchY, z, blockState);
                heightmap1 = null;
                heightmapsToUpdate--;
            }
            if (heightmap2 != null && blockPredicate2.test(blockState)) {
                heightmap2.update(x, searchY, z, blockState);
                heightmap2 = null;
                heightmapsToUpdate--;
            }
            if (heightmap3 != null && blockPredicate3.test(blockState)) {
                heightmap3.update(x, searchY, z, blockState);
                heightmap3 = null;
                heightmapsToUpdate--;
            }
        }
        if (heightmap0 != null) {
            heightmap0.update(x, bottomY - 1, z, worldChunk.getBlockState(mutable.set(x, bottomY - 1, z)));
        }
        if (heightmap1 != null) {
            heightmap1.update(x, bottomY - 1, z, worldChunk.getBlockState(mutable.set(x, bottomY - 1, z)));
        }
        if (heightmap2 != null) {
            heightmap2.update(x, bottomY - 1, z, worldChunk.getBlockState(mutable.set(x, bottomY - 1, z)));
        }
        if (heightmap3 != null) {
            heightmap3.update(x, bottomY - 1, z, worldChunk.getBlockState(mutable.set(x, bottomY - 1, z)));
        }
    }
}
