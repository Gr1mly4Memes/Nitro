package gr1mly4memes.nitro.async.tracking;
...
import gr1mly4memes.nitro.util.NamedAgnosticThreadFactory;
...
public class MultithreadedTracker {
    ...
    public static final ThreadPoolExecutor TRACKER_EXECUTOR = NitroConfig.multithreadedEnabled ? new ThreadPoolExecutor(
        getCorePoolSize(),
        getMaxPoolSize(),
    ...
    public static void tick(ServerLevel level) {
        try {
            if (!NitroConfig.multithreadedCompactModeEnabled) {
                tickAsync(level);
            } else {
    ...
    private static int getMaxPoolSize() {
        return NitroConfig.asyncEntityTrackerMaxThreads;
    }

    private static long getKeepAliveTime() {
        return NitroConfig.asyncEntityTrackerKeepalive;
    }
