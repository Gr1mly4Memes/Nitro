...
import gr1mly4memes.nitro.NitroLogger;
import gr1mly4memes.nitro.config.ConfigConvert;
import gr1mly4memes.nitro.config.ConfigVerify;
import gr1mly4memes.nitro.config.GlobalConfig;
import gr1mly4memes.nitro.config.RemovedConfig;
...
public class GlobalConfigManager {
            ...
            if (Modifier.isStatic(field.getModifiers())) {
                ...
                if (globalConfig != null) {
                    try {
                        ...
                        NitroConfig.config.addDefault(verifiedConfig.path, defValue);

                        try {
                            Object savedValue = NitroConfig.config.get(verifiedConfig.path);
                            if (isEnumConfig) {
                            ...
                            NitroConfig.config.set(verifiedConfig.path, defValue);
                            NitroLogger.LOGGER.warning(e.getMessage() + ", reset to " + defValue);
                        }
    ...
    public record RemovedVerifiedConfig(RemovedConfig config, ConfigConvert<? super Object> convert, Field field,
        ...
        public void run() {
            if (config.transform()) {
                if (NitroConfig.config.contains(path)) {
                    String string = NitroConfig.config.get(path).toString();
                    try {
            ...
            NitroConfig.config.set(path, null);
        }
    ...
    public record VerifiedConfig(GlobalConfig config, ConfigVerify<? super Object> verify, Field field, String path) {
        ...
        public void set(String realValue) throws IllegalArgumentException {
            ...
            try {
                ...
                NitroConfig.config.set(path, verify instanceof ConfigVerify.EnumConfigVerify ? realValue.toUpperCase(Locale.ROOT) : value);
                NitroConfig.save();
        ...
        public Object get() {
            try {
                ...
                NitroLogger.LOGGER.log(Level.SEVERE, "Failure to get " + path + " value", e);
                return "<VALUE ERROR>";
            }
        }
